...
module.exports = function (source, config) {
  source = parseVirtualComponent(source, 'x-icon', function (query) {
    ...
    //let svgPath = path.resolve(config.options.projectRoot, `node_modules/zkui/src/icons/${type}.svg`)
    let svgPath = path.resolve(config.options.projectRoot, `src/assets/svg/${type}.svg`)
